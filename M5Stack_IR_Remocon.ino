...
void setup() {
  ...
  BLEServer *pServer = createServer("M5Stack IR Remocon", &connected);
...
void loop() {
  ...
  if (M5.BtnC.wasReleased()) {
    ...
    uint32_t data = 0x400401640A6F;
    uint16_t nbits = 0x0030;
    uint16_t repeat = 0x0003;
    irsend.sendPanasonic(address, data, nbits, repeat);
    M5LcdPrint( "SEND PANA: ", address, data, nbits, repeat);
    SerialPrint("SEND PANA: ", address, data, nbits, repeat);   
  }
  delay(300);
  M5.update();
}
